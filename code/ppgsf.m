...
%PPGSF propogates shadow scale field from scale scales
%
...
imhw = size(smsk); vlen = numel(bl.l);
...
eas = zeros(vlen,3); % end scale of each sampling line
...
for i = 1:3
    F = scatteredInterpolant(cxy(:,1),cxy(:,2),as(:,i),'linear');
    timg = NaN(imhw);
    timg(pidx) = F(py,px); % fill penumra scale from sparse scales
